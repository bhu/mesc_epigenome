...
# region
reg <- GRanges('chr11', IRanges(73.018e6, 73.11e6))
ttl <- 'chr11: 73.02 - 73.11mb'
xbrks <- seq(73.03e6, 73.11e6, 3e4)
xlabs <- sprintf('%.2fmb', xbrks / 1e6)
xlims <- c(start(reg), end(reg))
rcts <- tibble(start = c(73018988, 73047867),
               end = c(73042073, 73083579),
               kind = c('inactive', 'active'))
...
# import bigwigs
odr <- c('H3K36me3', 'H3K36me2', 'H3K27me3', 'H3K27me2', 'H3K27me1', 'mCG')
clr <- '#0C8140'
clrs <- c('intergenic' = '#FBB040',
          'active' = '#1C75BC',
          'inactive' = '#7F3F98')

bws <- list.files('../tracks', full.names = T, pattern = 'Par') %>%
...
pd <- read_csv('../data/libsz.csv') %>%
  ...
  mutate(ttl = ttl,
         ...
         mark = factor(sub('.*_', '', samp), odr)) %>%
  merge(ms, by = 'mark', all.x = T) %>% 
  arrange(mark) %>%
...
pd %>%
  ...
  geom_area(alpha = 1, color = NA, fill = clr) +
  scale_x_continuous(expand = expansion(0),
                     breaks = xbrks,
                     limits = xlims) +
  facet_grid2(mark ~ ., scales = 'free_y', switch = 'y',
              strip = strip_themed(
                text_y = elem_list_text(color = rep(clr, 6))
              )) +
  scale_y_continuous(expand = expansion(c(0, .1)), position = 'right') +
...
{ wrap_plots(p1, p2, p3, ncol = 1, heights = c(7, 1, 2)) &
    theme(plot.background = element_blank()) } %>%
  ggsave('f1_a.pdf', ., height = 3.4, width = 6, bg = 'transparent')

...
# region
reg <- GRanges('chr10', IRanges(44.97e6,46.71e6))
ttl <- 'chr10: 44.97 - 46.71mb'
xbrks <- seq(45e6, 46e6, 5e5)
xlabs <- sprintf('%.1fmb', xbrks / 1e6)
xlims <- c(start(reg), end(reg))
rcts <- tibble(start = c(45067216, 45376619, 45577829, 45.85e6),
               end = c(45162751, 45470201, 45712345, 46.71e6),
               kind = c('active', 'active', 'active', 'intergenic'))
...
{ wrap_plots(p1, p2, p3, ncol = 1, heights = c(13, 1, 2))  &
    theme(plot.background = element_blank()) } %>%
  ggsave('f1_c.pdf', ., height = 6, width = 6.5, bg = 'transparent')

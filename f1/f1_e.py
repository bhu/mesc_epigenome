...
clrs = {'WT': '#0C8140', 'sgNSD1': '#EE2E2E'}
samps = ['WT', 'sgNSD1']
chips = ['mESC_Par_H3K27me3', 'mESC_sgNSD1_H3K27me3']
inps = ['mESC_Par_input', 'mESC_sgNSD1_input']
conds = ['mESC_A1_par', 'mESC_A1_sgNSD1']
marks = ['H3K27me3', 'H3K27me3']

ext = '.silentParNsd1.point.mat.gz'
ms = []
k = 80
nrm = 'ms'
kind = 'lfc'
...
fig = plt.figure(constrained_layout=True,figsize=[4.9,4.5])
...
for i in np.arange(len(samps)):
    ...
    ax.set_xticklabels(['-20kb','Silent\npromoter','+20kb'])
    im = ax.imshow(mat.values, aspect='auto', cmap='magma',
                   vmin = zmi, vmax = zma)
cax = fig.add_subplot(gs[:,-1])
fig.colorbar(im, cax=cax, orientation="vertical")
plt.savefig('f1_e.%s.%s.pdf' % (nrm, kind), transparent=True,dpi=600)
plt.close()

...
# region
reg <- GRanges('chr4', IRanges(136.26e6,136.46e6))
ttl <- 'chr4: 136.26 - 136.46mb'
xbrks <- seq(136.3e6, 136.45e6, 5e4)
xlabs <- sprintf('%.2fmb', xbrks / 1e6)
xlims <- c(start(reg), end(reg))
rcts <- tibble(start = c(136441295, 136368281),
               end = c(136445115, 136394816),
               kind = c('inactive'))
...
# import bigwigs
odr <- c('EZH2','H3K27me3','mCG')
clrs2 <- c('WT' = '#0C8140','DNMT-TKO' = '#3953A4', 'sgNSD1' = '#EE2E2E')
clrs <- c('intergenic' = '#FBB040',
          'active' = '#1C75BC',
          'inactive' = '#7F3F98')

bws <- list.files('../tracks', full.names = T, pattern = 'EZH|27me3|mCG') %>%
  grep('36me|E36|Eed|rep2|A1|EZH2_KO|GSM|NSD1_KO|WT|sgNSD1_mCG', ., value = T, invert = T) %>%
  ...
  lapply(function(s) {
    lapply(odr, function(x) {
      ...
      scale_y_continuous(limits = c(0, m),
                         ...
                         position = 'right') %>%
        list() %>%
        rep(length(unique(na.omit(as.character(pd$cond))[pd$mark == x])))
...
pd %>%
  ...
  facet_grid2(track ~ ., scales = 'free_y', switch = 'y',
              strip = strip_themed(
                text_y = elem_list_text(color = unname(clrs2[c(1,1,2,3,1)]))
              )) +
...
{ wrap_plots(p1, p2, p3, ncol = 1, heights = c(5, 1, 2))  &
    theme(plot.background = element_blank()) } %>%
  ggsave('f4_a.pdf', ., height = 4, width = 9, bg = 'transparent')

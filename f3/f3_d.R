...
# region
reg <- GRanges('chr7', IRanges(66.13e6,66.79e6))
ttl <- 'chr7: 66.13 - 66.79mb'
xbrks <- seq(66.2e6, 66.7e6, 2e5)
xlabs <- sprintf('%.1fmb', xbrks / 1e6)
xlims <- c(start(reg), end(reg))
rcts <- tibble(start = c(66170150),
               end = c(66643206),
               kind = c('inactive'))
...
bws <- list.files('../tracks', full.names = T, pattern = '27me3') %>%
  grep('rep2|J1|NSD1_KO', ., value = T) %>%
...
{ wrap_plots(p1, p2, p3, ncol = 1, heights = c(4, 1, 2))  &
    theme(plot.background = element_blank()) } %>%
  ggsave('f3_d.pdf', ., height = 2.5, width = 8.4, bg = 'transparent')

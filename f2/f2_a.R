...
# region
reg <- GRanges('chr11', IRanges(19.7e6,22.4e6))
ttl <- 'chr11: 19.7 - 22.4mb'
xbrks <- seq(20e6, 22e6, 1e6)
xlabs <- sprintf('%.0fmb', xbrks / 1e6)
xlims <- c(start(reg), end(reg))
rcts <- tibble(start = c(19924442, 20542937, 21091324, 21994764),
               end = c(20332713, 20741556, 21370629, 22286795),
               kind = c('active', 'active', 'active', 'active'))
...
# import bigwigs
odr <- c('H3K36me2', 'DNMT3A', 'mCG', 'mCHH')
clrs2 <- c('WT' = '#0C8140', 'sgNSD1' = '#EE2E2E')
clrs <- c('intergenic' = '#FBB040',
          'active' = '#1C75BC',
          'inactive' = '#7F3F98')

bws <- list.files('../tracks', full.names = T, pattern = 'Par|sgNSD1') %>%
  grep('36me2|DNMT3A|_m', ., value = T) %>%
...
pd %>%
  ...
  ylab('MS/depth norm coverage or mC%') +
...
{ wrap_plots(p1, p2, p3, ncol = 1, heights = c(13, 1, 2))  &
    theme(plot.background = element_blank()) }  %>%
  ggsave('f2_a.pdf', ., height = 4, width = 12, bg = 'transparent')
